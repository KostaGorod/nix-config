    ...
    in
    {
      options.programs.mem0 = {
        ...
        userId = lib.mkOption {
          ...
          default = null;
          description = "Memory user ID; null uses the runtime user name";
        };
      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf cfg.enable {
          ...
          // lib.optionalAttrs (cfg.userId != null) {
          ...
          ++ lib.optionals cfg.enableMcpServer [
            (pkgs.writeShellScriptBin "mem0-mcp-server" ''
              ...
              export MEM0_DEFAULT_USER_ID="''${MEM0_DEFAULT_USER_ID:-${
                if cfg.userId == null then "$USER" else cfg.userId
              }}"
